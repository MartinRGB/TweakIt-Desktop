...
var InputTree = react_1.memo(function (_a) {
    ...
    var _b = react_1.useContext(AnimatorTypeContext_1.AnimatorTypeContext), setDurationData = _b.setDurationData, previousDataRange = _b.previousDataRange, currentDataRange = _b.currentDataRange, setCurrentSolverDataByIndex = _b.setCurrentSolverDataByIndex, currentSolverData = _b.currentSolverData, previousDataMin = _b.previousDataMin, currentAnimCalculator = _b.currentAnimCalculator, previousSolverData = _b.previousSolverData;
    ...
    var prevSolverData = (previousSolverData[index] === undefined) ? min : (previousSolverData[index]);
    var currSolverData = (currentSolverData[index] === undefined) ? defaultVal : currentSolverData[index];
    var prevSolverRange = (previousDataRange[index] === undefined) ? 1 : (previousDataRange[index]);
    var prevSolverMin = (previousDataMin[index] === undefined) ? 0 : (previousDataMin[index]);
    var transedPrevSolverData = min + (prevSolverData - prevSolverMin) / prevSolverRange * (max - min);
    var _f = react_1.useState(min + transedPrevSolverData), previousRangeValue = _f[0], setPreviousRangeValue = _f[1];
    var _g = react_1.useState(currSolverData), targetRangeValue = _g[0], setTargetRangeValue = _g[1];
