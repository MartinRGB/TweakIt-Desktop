...
import { AdbKitChangesSet, AdbKitClient, AdbKitTracker, PushTransfer} from '../../server/@devicefarmer/adbkit'
import { EventEmitter } from 'events';
import { spawn } from 'child_process';
import * as path from 'path';
import { DeviceDescriptor } from '../../server/device-info/DeviceDescriptor';
import { ARGS_STRING } from '../../server/ServerConstants';
...
import { NetInterface } from '../../server//interfaces/NetInterface';
...
function getPath(){
    ...
    if (process.env.NODE_ENV === 'development') {
        DIR = localAssetsPath + 'scrcpy-server/vendor/Genymobile/scrcpy'; //cast
    }else{
        DIR = appResPath + 'scrcpy-server/vendor/Genymobile/scrcpy'; //cast
    }
    return DIR;
}

const FILE_DIR = getPath();
